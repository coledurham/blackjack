...
const checkWinner = (store, next, action) => {
    ...
    if(dealerHandVal === 21){
        store.dispatch({type: types.UPDATE_WINNER, data: { winner: 'dealer'}})
    ...
    if(playerHandVal === 21)
    {
        store.dispatch({type: types.UPDATE_WINNER, data: { winner: 'player'}})
        store.dispatch({type: types.UPDATE_SCORE, data: {}})
    ...
    store.dispatch({type: types.UPDATE_PLAYER, data: { player: 'player'}})
